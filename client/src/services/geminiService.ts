...
export async function findRecipesWithAI(userIngredients: string, allRecipes: Recipe[]): Promise<AIResult> {
  ...
  const prompt = `
אתה עוזר בישול. המשתמש כתב את החומרים שיש לו: "${userIngredients}"

להלן רשימת מתכונים קיימים (JSON):
${JSON.stringify(recipesForAI, null, 2)}

המשימה שלך:
1. בדוק כל מתכון — אם המשתמש יכול להכין אותו עם רוב החומרים שיש לו (מקסימום 2 חומרים חסרים), הוסף אותו לרשימת "found".
2. אם אין מתכונים מתאימים, צור מתכון חדש מהחומרים שהמשתמש כתב.
3. החזר תמיד JSON בפורמט הזה בלבד, ללא טקסט נוסף:
